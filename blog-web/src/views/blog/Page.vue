...
const pageSlug = computed(() => props.slug || route.params.slug)

async function load() {
  const slug = pageSlug.value
  if (!slug) { rawContent.value = ''; loading.value = false; return }
  loading.value = true
  try {
    const res = await getPublicPageBySlug(slug)
    if (res.code === 200 && res.data) {
      rawContent.value = res.data.content
      document.title = res.data.title + ' - ' + (document.title.split(' - ')[1] || 'EchoPress')
    } else {
      rawContent.value = ''
    }
...
<style scoped>
.page-view { width: min(800px, 100%); margin: 0 auto; }
